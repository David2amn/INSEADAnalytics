...
#Run model and plot results [formation]
fit.f.log.clust<-glm(log(formation_energy_ev_natom+1)~0+.-bandgap_energy_ev-id+cluster_gamma*lattice_angle_gamma_degree + cluster_gamma*I(lattice_angle_gamma_degree^2) - cluster_gamma, data=training2, family=gaussian(link = "identity"))
predicted.f.log.clust<-exp(predict(fit.f.log.clust,testing2) )-1
plot(predicted.f.log.clust ~ testing2$formation_energy_ev_natom)
training2$formation_energy_pred <- predict(fit.f.log.clust, newdata = training2)
testing2$formation_energy_pred <- predict(fit.f.log.clust, newdata = testing2)

#Run model and plot results [bandgap]
fit.b.log.clust<-glm(log(bandgap_energy_ev+1)~0+.-formation_energy_ev_natom-id+cluster_gamma*lattice_angle_gamma_degree + cluster_gamma*I(lattice_angle_gamma_degree^2)  - cluster_gamma, data=training2, family=gaussian(link = "identity"))
predicted.b.log.clust<-exp(predict(fit.b.log.clust,testing2) )-1
plot(predicted.b.log.clust ~ testing2$bandgap_energy_ev)

#Calculate and show percentage error
percent.errors.f.log.clust <- sqrt(mean((log(predicted.f.log.clust+1) -log(testing2$formation_energy_ev_natom+1))^2))
percent.errors.b.log.clust <- sqrt(mean((log(predicted.b.log.clust+1) -log(testing2$bandgap_energy_ev+1))^2))
...
#Run model and plot results [formation]
fit.f.log.clust.interact<-glm(log(formation_energy_ev_natom+1)~0+.^2-bandgap_energy_ev-id+cluster_gamma*lattice_angle_gamma_degree + cluster_gamma*I(lattice_angle_gamma_degree^2) - cluster_gamma, data=training2, family=gaussian(link = "identity"))
predicted.f.log.clust.interact<-exp(predict(fit.f.log.clust.interact,testing2) )-1
plot(predicted.f.log.clust.interact ~ testing2$formation_energy_ev_natom)
training2$formation_energy_pred <- predict(fit.f.log.clust.interact, newdata = training2)
testing2$formation_energy_pred <- predict(fit.f.log.clust.interact, newdata = testing2)

#Run AIC on model [formation]
fit.f.log.clust.interac.AIC<-stepAIC(fit.f.log.clust.interact,direction = c("both"),trace = 1)
predicted.f.log.clust.interac.AIC<-predict(fit.f.log.clust.interac.AIC,testing2) 
plot(predicted.f.log.clust.interac.AIC ~ testing2$formation_energy_ev_natom)


#Run model and plot results [bandgap]
fit.b.log.clust.interact<-glm(log(bandgap_energy_ev+1)~0+.^2-formation_energy_ev_natom-id+cluster_gamma*lattice_angle_gamma_degree + cluster_gamma*I(lattice_angle_gamma_degree^2)  - cluster_gamma, data=training2, family=gaussian(link = "identity"))
predicted.b.log.clust.interact<-exp(predict(fit.b.log.clust.interact,testing2) )-1
plot(predicted.b.log.clust.interact ~ testing2$bandgap_energy_ev)

#Run model and plot results [formation]
fit.b.log.clust.interac.AIC<-stepAIC(fit.b.log.clust.interact,direction = c("both"),trace = 1)
predicted.b.log.clust.interac.AIC<-predict(fit.b.log.clust.interac.AIC,testing2) 
plot(predicted.b.log.clust.interac.AIC ~ testing2$bandgap_energy_ev)

#Calculate and show percentage error
percent.errors.f.log.clust.interact.AIC <- sqrt(mean((log(predicted.f.log.clust.interac.AIC+1) -log(testing2$formation_energy_ev_natom+1))^2))
percent.errors.b.log.clust.interact.AIC <- sqrt(mean((log(predicted.b.log.clust.interac.AIC+1) -log(testing2$bandgap_energy_ev+1))^2))
mean.score.log.clust.interact.AIC<- (percent.errors.f.log.clust.interact+percent.errors.b.log.clust.interact) /2
